...
if($response === false) {
  ...
  foreach($parser->entidades->entidad->filas->fila->columnas->columna as $msp){ 
    ...
    }else if($msp->campo=="direccion"){

      $direccion= $array= explode(":",$msp->valor);

      $jason['direccion']=$direccion[0];

    }else if($msp->campo=="telefono"){

      $telefono= $array= explode(":",$msp->valor);

      $jason['telefono']=$telefono[0];
